  ...
  <div class="register-container">
    ...
    <div class="register">
      ...
      <div class="content">
        ...
        <button @click="getvalidation">获取验证码</button>
        <span class="error-msg">错误提示信息</span>
      </div>
      <div class="content">
        <label>登录密码:</label>
...
export default {
  ...
  methods: {
    async getvalidation() {
      try {
        const { phone } = this
        phone && (await this.$store.dispatch('getvalidationcode', this.phone))
    ...
    async registered() {
      ...
      try {
        let message = ''
        if (phone && code && password && againpassword == password) {
          if (agree) {
            message = await this.$store.dispatch('sendregistered', data)
            alert(message)
